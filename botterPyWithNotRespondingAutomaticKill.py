	...
	while True:
		...
		try:
			...
			for i in range(len(r)):
				s = r[i]
				if(name in r[i].lower() and "responding" in r[i].lower()):
					stripped = r[i].split()
					print("Killed a non-responding window!")
					os.kill(int(stripped[1]), 9)
		except:
...
def readAccounts():
	with open("BotAccounts.txt") as fp:
		...
		while line:
			...
			#
			#runAlts(accountAsList[0],accountAsList[1])
			line = fp.readline()
...
def periodicalRestart():
	...
	while True:
		restartSeconds = random.uniform(3000, 4500)
		#restartSeconds = 30
...
#launchAccount("jasperalt1.mc@gmail.com", "iamawsome123")


#closeAll("MinecraftLauncher.exe")
